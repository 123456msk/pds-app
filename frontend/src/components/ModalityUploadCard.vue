<template>
  <section class="modality-upload" :class="[type, { dragging: isDragging, busy: state.isScanning }]" @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false" @drop.prevent="handleDrop">
    ...
    <div class="file-metrics"><span><b>{{ state.fileCount }}</b> 已选</span><span><b>{{ displayDicomCount }}</b> {{ type === 'mri' ? 'T2W DICOM' : 'DICOM' }}</span><span><b>{{ state.zipCount }}</b> ZIP</span><span><b>{{ displaySkippedCount }}</b> 跳过</span></div>
...
const displayDicomCount=computed(()=>props.type==='mri'?(state.value.selectedSeries?.count||0):state.value.dicomCount);
const displaySkippedCount=computed(()=>{
  if(props.type!=='mri')return state.value.skippedCount;
  const selectedCount=state.value.selectedSeries?.count||0;
  return Math.max(0,state.value.dicomCount-selectedCount)+state.value.skippedCount;
});
