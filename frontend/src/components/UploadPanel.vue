<template>
  <section class="workspace">
    <el-steps :active="activeStep" align-center finish-status="success" class="workflow-steps"><el-step title="上传影像" /><el-step title="前列腺分割" /><el-step title="AI 诊断" /></el-steps>
    ...
    <section class="clinical-workbench segmentation-workbench">
      <header class="workbench-header"><div><span class="section-index">02</span><div><h2>前列腺分割</h2><p>设置 MRI 与 CT 前列腺切片范围，提交后执行自动分割与跨模态映射。</p></div></div><span class="privacy-note">完整 T2W MRI 用于分割</span></header>
    ...
    <section v-if="hasCompletedResults" class="result-actions"><div><el-icon><CircleCheckFilled /></el-icon><div><strong>分割与映射已完成</strong><span>{{ completedPatientLabel }} · 已生成 12 个 NIfTI 文件</span></div></div><div class="result-action-buttons"><el-button :icon="Download" @click="downloadResults">下载结果</el-button><el-button type="primary" :icon="View" @click="openStandaloneViewer">打开阅片器</el-button><el-button type="success" :icon="DataAnalysis" :loading="diagnosisLoading" @click="startDiagnosis">开始诊断</el-button><el-button :icon="RefreshRight" @click="restartFlow">重新开始</el-button></div></section>

    <DiagnosisReport v-if="diagnosisStarted" id="diagnosis-report" :patient="completedPatient" :case-id="preparedResult?.case_id || ''" :slice-counts="sliceCounts" :prediction="diagnosisResult" :loading="diagnosisLoading" :error="diagnosisError" />
...
import { predictPreparedCase, prepareCaseOnBackend, segmentPreparedCase } from '../api/casePreparation';
...
const activeStep = computed(() => diagnosisStarted.value ? 3 : hasCompletedResults.value ? 2 : (isSubmitting.value || seriesReady.value) ? 1 : 0);
